/* =========================================================
   ...
   - Numera posiciones
   - Resalta la tienda seleccionada
   ========================================================= */
...
function injectStoreAlertsModalStyles() {
  ...
  style.textContent = `
    ...
    .store-alerts-modal-backdrop {
      ...
      padding: 18px; background: rgba(15, 23, 42, 0.55); backdrop-filter: blur(8px);
    }
    .store-alerts-modal {
      width: min(860px, 100%); max-height: min(82vh, 760px); overflow: hidden;
      border-radius: 28px; background: #f8fafc;
      box-shadow: 0 24px 70px rgba(15, 23, 42, 0.32);
      border: 1px solid rgba(148, 163, 184, 0.35);
      display: flex; flex-direction: column;
    }
    .store-alerts-modal__header {
      padding: 20px 20px 14px; display: flex; align-items: flex-start; justify-content: space-between; gap: 14px;
      background: linear-gradient(135deg, #ffffff 0%, #fff7ed 100%);
      border-bottom: 1px solid rgba(148, 163, 184, 0.25);
    }
    .store-alerts-modal__title { margin: 0; font-size: 1.05rem; font-weight: 950; color: #0f172a; letter-spacing: -0.03em; }
    .store-alerts-modal__subtitle { margin: 5px 0 0; font-size: 0.84rem; color: #64748b; font-weight: 750; }
    ...
    .store-alerts-table th {
      padding: 0 10px 2px; text-align: left; font-size: 0.72rem; color: #64748b;
      text-transform: uppercase; letter-spacing: 0.04em; font-weight: 950; white-space: nowrap;
    }
    .store-alerts-table td {
      padding: 13px 10px; background: #ffffff;
      border-top: 1px solid rgba(226, 232, 240, 0.95);
      border-bottom: 1px solid rgba(226, 232, 240, 0.95); vertical-align: middle;
    }
    .store-alerts-table td:first-child { border-left: 1px solid rgba(226, 232, 240, 0.95); border-radius: 16px 0 0 16px; }
    .store-alerts-table td:last-child { border-right: 1px solid rgba(226, 232, 240, 0.95); border-radius: 0 16px 16px 0; }
    .store-alerts-row--selected td {
      background: #fff7ed;
    ...
    .store-alerts-position {
      display: inline-flex; align-items: center; justify-content: center;
      width: 30px; height: 30px; border-radius: 999px;
      background: #e2e8f0; color: #0f172a; font-weight: 950; font-size: 0.78rem;
    }
    .store-alerts-row--selected .store-alerts-position { background: #f97316; color: #ffffff; }
    .store-alerts-store { display: block; max-width: 310px; color: #0f172a; font-size: 0.86rem; font-weight: 950; line-height: 1.15; }
    .store-alerts-selected-label { display: block; margin-top: 4px; color: #ea580c; font-size: 0.69rem; font-weight: 950; text-transform: uppercase; }
    .store-alerts-total {
      display: inline-flex; align-items: center; justify-content: center; min-width: 44px;
      border-radius: 999px; padding: 8px 10px; background: #fee2e2; color: #991b1b;
      font-size: 0.94rem; font-weight: 950; white-space: nowrap;
    }
    .store-alerts-total--zero { background: #dcfce7; color: #166534; }
    .store-alerts-number { color: #0f172a; font-size: 0.95rem; font-weight: 950; white-space: nowrap; }
    .store-alerts-loading, .store-alerts-empty { padding: 32px 14px; text-align: center; color: #64748b; font-weight: 850; }
    @media (min-width: 720px) { .store-alerts-modal-backdrop { align-items: center; } }
    @media (max-width: 560px) {
      .store-alerts-modal-backdrop { padding: 10px; }
      .store-alerts-modal { border-radius: 24px 24px 18px 18px; max-height: 86vh; }
      .store-alerts-table th:nth-child(5), .store-alerts-table td:nth-child(5),
      .store-alerts-table th:nth-child(6), .store-alerts-table td:nth-child(6) { display: none; }
      .store-alerts-store { max-width: 135px; font-size: 0.79rem; }
      .store-alerts-number { font-size: 0.88rem; }
      .store-alerts-position { width: 28px; height: 28px; }
    }
  `;
  document.head.appendChild(style);
}

function normalizeAlertStoreNameForCompare(value) {
...
function renderStoreAlertsModalShell() {
  ...
  backdrop.innerHTML = `
    <section class="store-alerts-modal" role="dialog" aria-modal="true" aria-labelledby="storeAlertsModalTitle">
      ...
      <div class="store-alerts-modal__body" id="storeAlertsModalBody"><div class="store-alerts-loading">Cargando tiendas...</div></div>
    </section>
  `;
...
function renderStoreAlertsRows(rows) {
  ...
  body.innerHTML = `
    <table class="store-alerts-table">
      <thead><tr><th>Pos.</th><th>Tienda</th><th>Alertas</th><th>Sin venta</th><th>Críticas</th><th>Lentas</th></tr></thead>
      <tbody>
        ${rows.map((row, index) => {
          ...
          return `
            <tr class="${isSelected ? 'store-alerts-row--selected' : ''}">
              <td><span class="store-alerts-position">${index + 1}</span></td>
              <td><span class="store-alerts-store">${escapeHtml(row.tienda)}</span>${isSelected ? '<span class="store-alerts-selected-label">Tienda seleccionada</span>' : ''}</td>
...
(function initStoreAlertsModalWhenReady() {
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', setupStoreAlertsModal);
  else setupStoreAlertsModal();
