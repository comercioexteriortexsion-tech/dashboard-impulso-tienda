...
function injectStoreAlertsModalStyles() {
  ...
  style.textContent = `
    ...
    .store-alerts-empty { padding:32px 14px; text-align:center; color:#64748b; font-weight:850; line-height:1.35; }
    .store-alerts-empty small { display:block; margin-top:8px; color:#94a3b8; font-size:.76rem; font-weight:750; }
...
function hydrateAlertsRankingFromCache() {
  const cached = getInitialDashboardCacheForAlerts();
  if (Array.isArray(cached.ranking_alertas) && cached.ranking_alertas.length) {
    rankingAlertasTiendas = normalizeRankingAlertas(cached.ranking_alertas);
    return rankingAlertasTiendas;
  }
  return Array.isArray(rankingAlertasTiendas) ? rankingAlertasTiendas : [];
}
...
function openStoreAlertsModal() {
  ...
  const rows = hydrateAlertsRankingFromCache();
  ...
  if (!rows.length && typeof window.refreshDashboardRankings === 'function') {
    window.refreshDashboardRankings(false).then(function () {
      if (document.getElementById('storeAlertsModalBody')) {
        renderStoreAlertsRows(hydrateAlertsRankingFromCache() || []);
      }
    });
  }
}
...
function renderStoreAlertsRows(rows) {
  ...
  if (!rows.length) {
    body.innerHTML = '<div class="store-alerts-empty">El ranking se está actualizando en segundo plano.<small>El dashboard puede seguir usándose mientras termina la consulta.</small></div>';
    return;
  }
...
window.addEventListener('impulsoRankingsReady', function () {
  hydrateAlertsRankingFromCache();
  if (document.getElementById('storeAlertsModalBody')) renderStoreAlertsRows(rankingAlertasTiendas || []);
});
