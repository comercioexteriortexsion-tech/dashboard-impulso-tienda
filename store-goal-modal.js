...
function injectStoreGoalModalStyles() {
  ...
  style.textContent = `
    ...
    .store-goal-modal {
      width: min(820px, 100%); max-height: min(82vh, 760px); overflow: hidden;
    ...
    .store-goal-modal__title { margin: 0; font-size: 1.05rem; font-weight: 950; color: #0f172a; letter-spacing: -0.03em; }
    .store-goal-modal__subtitle { margin: 5px 0 0; font-size: 0.84rem; color: #64748b; font-weight: 750; }
    ...
    .store-goal-table td:first-child { border-left: 1px solid rgba(226, 232, 240, 0.95); border-radius: 16px 0 0 16px; }
    .store-goal-table td:last-child { border-right: 1px solid rgba(226, 232, 240, 0.95); border-radius: 0 16px 16px 0; }
    .store-goal-row--selected td {
      background: #fff7ed;
    ...
    .store-goal-row--selected .store-goal-position { background: #f97316; color: #ffffff; }
    .store-goal-store { display: block; max-width: 310px; color: #0f172a; font-size: 0.86rem; font-weight: 950; line-height: 1.15; }
    .store-goal-selected-label { display: block; margin-top: 4px; color: #ea580c; font-size: 0.69rem; font-weight: 950; text-transform: uppercase; }
    .store-goal-percent { display: inline-flex; align-items: center; gap: 5px; font-size: 0.98rem; font-weight: 950; white-space: nowrap; }
    .store-goal-percent--up { color: #15803d; }
    .store-goal-percent--down { color: #dc2626; }
    .store-goal-expected { color: #0f172a; font-size: 0.98rem; font-weight: 950; white-space: nowrap; }
    .store-goal-status {
      display: inline-flex; align-items: center; justify-content: center; min-width: 76px;
      border-radius: 999px; padding: 7px 10px; font-size: 0.72rem; font-weight: 950; white-space: nowrap;
    }
    .store-goal-status--up { background: #dcfce7; color: #166534; }
    .store-goal-status--down { background: #fee2e2; color: #991b1b; }
    .store-goal-loading, .store-goal-empty { padding: 32px 14px; text-align: center; color: #64748b; font-weight: 850; }
    @media (min-width: 720px) { .store-goal-modal-backdrop { align-items: center; } }
    @media (max-width: 560px) {
      .store-goal-modal-backdrop { padding: 10px; }
      .store-goal-modal { border-radius: 24px 24px 18px 18px; max-height: 86vh; }
      .store-goal-table th:nth-child(5), .store-goal-table td:nth-child(5) { display: none; }
      .store-goal-store { max-width: 135px; font-size: 0.79rem; }
      .store-goal-percent, .store-goal-expected { font-size: 0.9rem; }
      .store-goal-position { width: 28px; height: 28px; }
    }
  `;
  document.head.appendChild(style);
}

function normalizeStoreNameForCompare(value) {
...
function renderStoreGoalModalShell() {
  ...
  backdrop.innerHTML = `
    <section class="store-goal-modal" role="dialog" aria-modal="true" aria-labelledby="storeGoalModalTitle">
      ...
      <div class="store-goal-modal__body" id="storeGoalModalBody"><div class="store-goal-loading">Cargando tiendas...</div></div>
    </section>
  `;
...
async function getStoreGoalRows() {
  ...
  try {
    ...
    storeGoalRowsCache = rows.filter(row => row.tienda).sort((a, b) => b.cumplimiento - a.cumplimiento);
...
function renderStoreGoalRows(rows) {
  ...
  body.innerHTML = `
    <table class="store-goal-table">
      <thead><tr><th>Pos.</th><th>Tienda</th><th>Cómo va</th><th>Debería ir</th><th>Estado</th></tr></thead>
      <tbody>
        ${rows.map((row, index) => {
          ...
          return `
            <tr class="${isSelected ? 'store-goal-row--selected' : ''}">
              <td><span class="store-goal-position">${index + 1}</span></td>
              <td><span class="store-goal-store">${escapeHtml(row.tienda)}</span>${isSelected ? '<span class="store-goal-selected-label">Tienda seleccionada</span>' : ''}</td>
...
(function initStoreGoalModalWhenReady() {
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', setupStoreGoalModal);
  else setupStoreGoalModal();
